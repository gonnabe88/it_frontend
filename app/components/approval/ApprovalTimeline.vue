    ...
    <Dialog v-model:visible="isVisible" modal class="w-[90vw]" :style="{ maxWidth: '1000px' }" :show-header="false">
        ...
        <div class="p-6 bg-slate-50 dark:bg-zinc-900/50 overflow-x-auto rounded-b-lg">
            <div class="relative flex items-start justify-between min-w-[600px] px-4 pt-4 pb-8">
                ...
                <!-- 타임라인 각 항목 (기안자 + 결재자) -->
                <div
v-for="(approver, index) in timelineApprovers" :key="index"
                    class="relative z-10 flex flex-col items-center flex-1 min-w-[150px]">
                    <!-- 상태별 색상 원형 마커 -->
                    <div
class="w-10 h-10 rounded-full flex items-center justify-center border-4 border-white dark:border-zinc-900 shadow-sm transition-all duration-300 mb-4"
                        :class="{
