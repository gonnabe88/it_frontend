    ...
    <template v-if="editor">
        ...
        <div
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
v-for="level in [1, 2, 3, 4]" :key="level" class="tbar-btn font-bold text-xs"
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('bold') }"
                    title="굵게 (Ctrl+B)" @click="editor.chain().focus().toggleBold().run()">
                    <strong>B</strong>
                </button>
                <button
class="tbar-btn italic" :class="{ 'tbar-btn-active': editor.isActive('italic') }"
                    title="기울임 (Ctrl+I)" @click="editor.chain().focus().toggleItalic().run()">
                    I
                </button>
                <button
class="tbar-btn underline" :class="{ 'tbar-btn-active': editor.isActive('underline') }"
                    title="밑줄 (Ctrl+U)" @click="editor.chain().focus().toggleUnderline().run()">
                    U
                </button>
                <button
class="tbar-btn line-through" :class="{ 'tbar-btn-active': editor.isActive('strike') }"
                    title="취소선" @click="editor.chain().focus().toggleStrike().run()">
                    S
                </button>
            </div>
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn text-xs" :class="{ 'tbar-btn-active': editor.isActive('subscript') }"
                    title="아래첨자" @click="editor.chain().focus().toggleSubscript().run()">
                    X<sub>2</sub>
                </button>
                <button
class="tbar-btn text-xs" :class="{ 'tbar-btn-active': editor.isActive('superscript') }"
                    title="위첨자" @click="editor.chain().focus().toggleSuperscript().run()">
                    X<sup>2</sup>
                </button>
            </div>
            ...
            <div class="toolbar-group flex gap-0.5 items-center">
                ...
                <div class="relative">
                    <button
class="tbar-btn flex flex-col items-center justify-center gap-0 min-w-[28px]" title="글자 색상"
                        ...
                        <span class="w-4 h-[3px] rounded-sm" :style="{ backgroundColor: currentTextColor }"/>
                    </button>
                    <!-- 글자색 팔레트 패널 -->
                    <div
v-if="colorPaletteVisible"
                        ...
                        <button
                            ...
                            <i class="pi pi-times text-[10px] mr-1"/>색상 제거
                        </button>
...
v-for="color in row" :key="color"
                ...
                <div class="relative">
                    <button
class="tbar-btn flex flex-col items-center justify-center gap-0 min-w-[28px]" title="배경 색상"
                        ...
                        <i class="pi pi-pen-to-square text-[11px] leading-tight"/>
                        <span class="w-4 h-[3px] rounded-sm" :style="{ backgroundColor: currentHighlightColor }"/>
                    </button>
                    <!-- 배경색 팔레트 패널 -->
                    <div
v-if="highlightPaletteVisible"
                        class="absolute top-full left-0 z-50 mt-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-xl p-2"
                        style="width: 190px;">
                        <button
                            class="w-full text-left text-xs text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200 py-1 px-2 rounded hover:bg-zinc-100 dark:hover:bg-zinc-700 mb-1"
...
v-for="color in row" :key="color"
                ...
                <Teleport to="body">
                    <div
v-if="colorPaletteVisible || highlightPaletteVisible" class="fixed inset-0 z-40"
                        @click="colorPaletteVisible = false; highlightPaletteVisible = false" />
                </Teleport>

                <!-- 모든 서식 지우기 -->
                <button
class="tbar-btn" title="서식 지우기"
                    @click="editor.chain().focus().unsetAllMarks().clearNodes().run()">
                    <i class="pi pi-eraser text-xs"/>
                </button>
            </div>
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive({ textAlign: 'left' }) }"
                    title="왼쪽 정렬" @click="editor.chain().focus().setTextAlign('left').run()">
                    <i class="pi pi-align-left text-xs"/>
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive({ textAlign: 'center' }) }"
                    title="가운데 정렬" @click="editor.chain().focus().setTextAlign('center').run()">
                    <i class="pi pi-align-center text-xs"/>
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive({ textAlign: 'right' }) }"
                    title="오른쪽 정렬" @click="editor.chain().focus().setTextAlign('right').run()">
                    <i class="pi pi-align-right text-xs"/>
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive({ textAlign: 'justify' }) }"
                    title="양쪽 정렬" @click="editor.chain().focus().setTextAlign('justify').run()">
                    <i class="pi pi-align-justify text-xs"/>
                </button>
            </div>
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('bulletList') }"
                    title="글머리 기호" @click="editor.chain().focus().toggleBulletList().run()">
                    <i class="pi pi-list text-xs"/>
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('orderedList') }"
                    title="번호 목록" @click="editor.chain().focus().toggleOrderedList().run()">
                    <span class="text-xs font-bold">1.</span>
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('taskList') }"
                    title="체크리스트" @click="editor.chain().focus().toggleTaskList().run()">
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('blockquote') }"
                    title="인용문" @click="editor.chain().focus().toggleBlockquote().run()">
                    <i class="pi pi-comment text-xs"/>
                </button>
                <button
class="tbar-btn font-mono text-xs" :class="{ 'tbar-btn-active': editor.isActive('code') }"
                    title="인라인 코드" @click="editor.chain().focus().toggleCode().run()">
                    &lt;/&gt;
                </button>
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('codeBlock') }"
                    title="코드 블록" @click="editor.chain().focus().toggleCodeBlock().run()">
            ...
            <div class="toolbar-group flex gap-0.5">
                <!-- 링크 -->
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('link') }" title="링크 삽입"
                    @click="openLinkDialog">
                    <i class="pi pi-link text-xs"/>
                </button>
                <!-- 링크 제거 -->
                <button
v-if="editor.isActive('link')" class="tbar-btn"
                    title="링크 제거" @click="editor.chain().focus().unsetLink().run()">
                    <i class="pi pi-times text-xs"/>
                </button>
                <!-- 이미지 -->
                <button class="tbar-btn" title="이미지 삽입" @click="openImageDialog">
                    <i class="pi pi-image text-xs"/>
                </button>
                <!-- 표 삽입 (플로팅 툴바로 행/열 조작) -->
                <button
class="tbar-btn" :class="{ 'tbar-btn-active': editor.isActive('table') }" title="표 삽입"
                    @click="insertTable">
                    <i class="pi pi-table text-xs"/>
                </button>
                <!-- 구분선 -->
                <button class="tbar-btn" title="구분선" @click="editor.chain().focus().setHorizontalRule().run()">
            ...
            <!-- 폰트 패밀리 -->
            <select
class="tbar-select" :value="selectedFont"
                title="폰트 선택" @change="applyFontFamily(($event.target as HTMLSelectElement).value)">
                <option v-for="f in fontOptions" :key="f.value" :value="f.value">{{ f.label }}</option>
            </select>

            <!-- 글자 크기 (FR-04) -->
            <select
class="tbar-select" :value="selectedFontSize"
                title="글자 크기" @change="applyFontSize(($event.target as HTMLSelectElement).value)">
            ...
            <div class="toolbar-group flex gap-0.5">
                <button
class="tbar-btn" :disabled="!editor.can().undo()" title="실행 취소 (Ctrl+Z)"
                    @click="editor.chain().focus().undo().run()">
                    <i class="pi pi-undo text-xs"/>
                </button>
                <button
class="tbar-btn" :disabled="!editor.can().redo()" title="다시 실행 (Ctrl+Y)"
            ...
            <button
                ...
                title="Excalidraw 다이어그램 삽입" @click="insertExcalidraw">
        ...
        <Dialog v-model:visible="linkDialogVisible" modal header="링크 삽입" :style="{ width: 'var(--dialog-sm)' }">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex items-center gap-2">
                    <Checkbox v-model="linkOpenInNewTab" :binary="true" input-id="linkNewTab" />
...
v-if="isUploadingImage"
                            ...
                            <i class="pi pi-spin pi-spinner text-indigo-500"/>
                            <span>이미지 업로드 중...</span>
                        </div>
                        <input
v-else ref="imageFileInput" type="file" accept="image/*"
...
v-if="imageTab === 'url'" label="삽입" icon="pi pi-image" :disabled="!imageUrl"
        ...
        <!-- ── Excalidraw 편집 다이얼로그 ── -->
        <Dialog
            :visible="isExcalidrawOpen" modal header="다이어그램 편집"
            :style="{ width: '92vw', maxWidth: '1400px' }" :content-style="{ padding: '0', height: 'calc(90vh - 120px)', display: 'flex', flexDirection: 'column' }"
            @update:visible="(val) => { if (!val) closeExcalidraw(); }"
            @hide="closeExcalidraw"
            @after-show="handleExcalidrawAfterShow">
            <div class="flex-1 min-h-0 h-full">
                <ClientOnly>
                    <ExcalidrawWrapper
v-if="isExcalidrawOpen" ref="excalidrawWrapperRef"
                        :initial-scene-data="parsedExcalidrawInitialData" class="block h-full" />
                    <template #fallback>
                        <div class="flex items-center justify-center h-full text-zinc-400">
                            <div class="text-center">
                                <i class="pi pi-spin pi-spinner text-3xl mb-3 block"/>
        ...
        <Dialog v-model:visible="fileAttachDialogVisible" modal header="첨부파일 관리" :style="{ width: 'var(--dialog-lg)' }">
            <div class="flex flex-col gap-4">
                ...
                <div
                    ...
                    <Button
label="업로드" icon="pi pi-upload" size="small" :loading="isUploadingFile"
...
class="text-sm font-medium text-zinc-800 dark:text-zinc-200 truncate"
                            ...
                            <div
                                ...
                                <Button
icon="pi pi-trash" label="삭제" size="small" severity="danger" text
                                    :disabled="!fileDeleteFn" @click="confirmDeleteAttachment(file)" />
                                <Button
icon="pi pi-plus" label="본문 삽입" size="small" severity="secondary"
...
v-else
                        ...
                        <i class="pi pi-folder-open text-3xl text-zinc-300 mb-3"/>
