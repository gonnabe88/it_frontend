...
<template>
    <Dialog
v-model:visible="isVisible" :header="header" modal :style="{ width: '70vw', height: '70vh' }"
        :breakpoints="{ '960px': '75vw', '641px': '90vw' }" content-class="h-full flex flex-col">
        <div class="h-full flex flex-col">
            <Splitter class="h-full border-none" style="min-height: 400px">
                <SplitterPanel :size="30" :min-size="20" class="h-full flex flex-col overflow-auto p-2">
                    ...
                    <Tree
v-model:selection-keys="selectedNode" v-model:expanded-keys="expandedKeys" :value="nodes"
                        selection-mode="single" class="w-full border-none p-0" :filter="true"
                        filter-mode="lenient" filter-placeholder="부서 검색" @node-select="onNodeSelect"/>
                </SplitterPanel>
                <SplitterPanel :size="70" :min-size="50" class="h-full flex flex-col p-2 relative">
                    <StyledDataTable
v-model:selection="selectedUser" :value="users" :loading="loadingUsers" size="small" striped-rows
                        selection-mode="single" data-key="eno" :paginator="true" :rows="10"
                        scrollable scroll-height="flex" @row-select="onUserSelect">
                        <template #empty>
                        ...
                        <Column field="bbrNm" header="부서" sortable style="width: 20%"/>
                        <Column field="temNm" header="팀" sortable style="width: 20%"/>
                        <Column field="usrNm" header="이름" sortable style="width: 20%"/>
                        <Column field="eno" header="사번" sortable style="width: 20%"/>
