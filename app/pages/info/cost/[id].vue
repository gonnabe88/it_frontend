...
const { getCodeName: getDfrCleName } = useCodeOptions('DFR_CLE');
...
/** 금융정보단말기 유형 여부 (IT_MNGC_TP_002) */
const isTerminalType = computed(() => cost.value?.itMngcTp === 'IT_MNGC_TP_002');
...
const tocItems = computed<TocItem[]>(() => [
    ...
    ...(isTerminalType.value ? [{ id: 'section-terminal', label: '금융정보단말기 상세목록', icon: 'pi pi-desktop' }] : [])
]);
...
onMounted(() => {
    ...
    observer = new IntersectionObserver((entries) => {
        ...
        const allIds = tocItems.value.flatMap(item => [item.id, ...(item.children?.map(c => c.id) || [])]);
            ...
            <template #title>
                <div class="space-y-1">
                    ...
                    <div class="flex flex-wrap items-center gap-2 text-sm text-zinc-500">
                        <Tag
:value="getPulDttName(cost.pulDtt)"
...
id="section-contract"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-950/30 rounded-xl">
                            ...
                            <span class="font-bold text-zinc-900 dark:text-zinc-100">{{ getPulDttName(cost.pulDtt) || '-' }}</span>
                        </div>
                        <div
...
:value="cost.infPrtYn === 'Y' ? '정보보호 대상' : '일반'"
                ...
                <!-- 금융정보단말기 상세목록 (IT_MNGC_TP_001인 경우에만 표시) -->
                <section
v-if="isTerminalType"
id="section-terminal"
                    class="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                    <h3 class="font-bold text-xl text-zinc-900 dark:text-zinc-100 flex items-center gap-2 mb-6">
                        <i class="pi pi-desktop text-indigo-500"/>
                        금융정보단말기 상세목록
                    </h3>
                    <StyledDataTable :value="cost.terminals ?? []" striped-rows data-key="tmnSno">
                        <Column field="tmnNm" header="단말기명" />
                        <Column field="tmnTuzManr" header="이용방법" />
                        <Column field="tmnUsg" header="용도" />
                        <Column field="tmnSvc" header="서비스" />
                        <Column field="tmlAmt" header="금액">
                            <template #body="{ data }">
                                <span class="text-right block">{{ formatCurrency(data.tmlAmt, data.cur) }}</span>
                            </template>
                        </Column>
                        <Column field="dfrCle" header="지급주기">
                            <template #body="{ data }">
                                <span>{{ getDfrCleName(data.dfrCle) }}</span>
                            </template>
                        </Column>
