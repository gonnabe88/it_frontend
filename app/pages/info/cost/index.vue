...
[일괄 수정 로직]
  - 현재 사용자(user.eno)와 cgpr(담당자 사원번호)이 일치하는 항목만 대상
...
const handleBulkEdit = () => {
    ...
    // cgpr(담당자 사원번호)이 현재 사용자와 일치하는 항목만 추출
    const myCosts = costs.value.filter(cost => cost.cgpr === user.value?.eno);
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <!-- 통합 검색 입력 (비목코드·계약명·계약상대처·담당부서·담당자) -->
                <IconField>
                    <InputIcon class="pi pi-search" />
                    <InputText
                        v-model="searchKeyword"
                        placeholder="비목코드, 계약명, 담당자 등 검색"
                        class="w-64"
                    />
                </IconField>
        ...
        <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <DataTable v-else :value="filteredCosts" paginator :rows="10" :rowsPerPageOptions="[10, 20, 50]" v-model:selection="selectedCosts"
                ...
                <Column field="ioeC" header="비목코드" sortable></Column>
                ...
                <Column field="biceDpmNm" header="담당부서" sortable></Column>
                <Column field="cgprNm" header="담당자" sortable></Column>
