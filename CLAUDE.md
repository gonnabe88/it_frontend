...
| 구분 | 함수 | 용도 |
|------|------|------|
| GET (조회) | `useApiFetch<T>` | `useFetch` 래퍼. 반응형 데이터 조회. httpOnly 쿠키 자동 전송 및 401 처리 포함. |
| POST/PUT/DELETE (변경) | `$apiFetch` | `$fetch` 래퍼. 일회성 데이터 변경. httpOnly 쿠키 자동 전송 및 401 처리 포함. |
...
// 주의: stores/auth.ts 내부에서는 $apiFetch 사용 불가 (순환 참조 발생)
// 로그인/갱신/로그아웃은 Nuxt 내장 $fetch + credentials:'include'를 직접 사용합니다.
```
...
- JWT는 프론트엔드에서 직접 저장하거나 읽지 않습니다. Access Token과 Refresh Token은 백엔드가 발급하는 httpOnly 쿠키로만 전달합니다.
- 모든 인증 API 요청은 `credentials: 'include'`를 유지합니다.
- 사용자 표시용 인증 상태는 `stores/auth.ts`의 `useCookie('it-portal-user')` 기반으로 관리합니다.
- 다크모드 초기 상태는 `theme-dark` 쿠키를 기준으로 `nuxt.config.ts` 인라인 스크립트에서 하이드레이션 전에 적용합니다.
...
- `composables/useBudgetPeriod.ts` - 예산 기준연도/기간 상태
- `composables/useBudgetStatus.ts` - 예산현황 3탭 조회
- `composables/useCodeOptions.ts` - 공통코드 옵션 조회
- `composables/useCost.ts` - 전산업무비 조회
- `composables/useCouncil.ts` - 정보화실무협의회 CRUD/상태 전이
- `composables/useCouncilCodes.ts` - 협의회 상태/심의유형 코드 조회
...
- `composables/useExcalidrawAttachment.ts` - Excalidraw 첨부파일 임시 ID 추적
- `composables/useExcalidrawDialog.ts` - Excalidraw 편집 다이얼로그 상태
- `composables/useFiles.ts` - 첨부파일 업로드/다운로드/미리보기
- `composables/useGlobalSearch.ts` - 전역 검색 상태 및 API
...
- `composables/usePendingApprovalCount.ts` - 대기 결재 배지 카운트 조회
...
- `composables/useReviewCommentApi.ts` - 사전협의 코멘트 서버 동기화
- `composables/useTableCellSelection.ts` - 엑셀 스타일 셀 선택/복사
- `composables/useTabs.ts` - 탭 상태 관리

**Stores / Utils / Types**
- `stores/auth.ts` - 로그인/로그아웃/토큰갱신/user 쿠키 기반 세션복원
...
- `utils/excel.ts` - Excel 내보내기 유틸리티
- `utils/hwpx.ts` - HTML→HWPX 변환 유틸리티
- `types/auth.ts` - 인증 타입 + ROLE 상수 (ADMIN/USER/DEPT_MANAGER)
- `types/budget-work.ts` - 예산 작업 타입 (편성비목/편성결과)
- `types/budgetStatus.ts` - 예산현황 조회 타입
- `types/council.ts` - 정보화실무협의회 타입
...
개발 완료 후 변경 범위에 따라 다음 명령을 실행합니다.
- 타입 변경/컴포넌트 변경: `npm run typecheck`
- 코드 스타일/정적 분석: `npm run lint`
- 단위 로직 변경: `npm test`
- 사용자 시나리오 변경: `npm run test:e2e`

2026-04-29 기준 단위 테스트는 12개 파일, 248개 케이스입니다.
`coverage/`, `test-results/`, `.nuxt/`, `.output/`, `dist/`는 생성물이므로 ESLint 대상에서 제외합니다.
...
```TypeScript
    /**
     * 구버전 세션 데이터 마이그레이션
     * useCookie 전환 전에 localStorage에 남아 있던 user 정보를
     * it-portal-user 쿠키로 1회 이전합니다.
     *
     * [현재 인증 전략]
     * - JWT 토큰은 httpOnly 쿠키로만 관리되어 JavaScript에서 접근하지 않습니다.
     * - 프론트엔드는 사용자 표시와 라우트 가드에 필요한 user 정보만 useCookie로 보관합니다.
     *
     * [예외 처리]
     * - JSON.parse 실패(데이터 손상) 시 값을 무시하고 localStorage 잔존 데이터를 삭제합니다.
     */
...
const updateProject = async (id: string, data: ProjectUpdatePayload): Promise<void> => {
  // 인증 쿠키가 자동 전송되도록 $apiFetch를 사용합니다.
  await $apiFetch(`/api/projects/${id}`, { method: 'PUT', body: data });
};
```

## 6. 문서 최신화 규칙

- `README.md`는 신규 개발자가 구조와 흐름을 파악하는 개발 노트로 유지합니다.
- `CLAUDE.md`는 코드 생성 규칙과 금지 패턴을 담는 실행 지침입니다.
- 인증, 테스트 수치, 주요 composable/type 목록이 바뀌면 두 문서를 함께 갱신합니다.
- 장기 TODO/FIXME는 루트 `TASK.md` 또는 `it_frontend/TASK.md`에 후속 과제로 등록합니다.
