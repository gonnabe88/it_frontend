...
- [x] 접속자 소속부서 기준 사업명 목록 표출 (프로젝트 테이블 주관부서 매핑)
- [x] 사업별 정보화실무협의회 진행상태 표출 (타당성검토표 작성중/완료, 협의회 개최준비/완료)
- [x] 타당성검토표 작성 폼 (사업개요 8항목, 타당성 자체점검 6항목, 성과관리 자체계획)
- [x] 성과지표 동적 추가/삭제 (기본 1개, 사용자가 추가 가능)
- [x] 임시저장 / 작성완료 버튼
- [x] 사업계획서 첨부파일 (hwp, hwpx, pdf 필수)
- [x] 작성완료 후 팀장 결재 프로세스 (전자결재 연동은 추후, 포털 내 결재로 구현)

**Step 2. 협의회 개최 준비 (IT관리자)**
- [x] 타당성검토표 작성완료(+결재완료) 사업 목록 표출
- [x] 심의 유형 선택 (정보시스템/정보보호/기타) → 당연위원 자동 매핑
- [x] 소집위원 및 간사 추가 (직원검색 팝업)
- [x] 평가위원 확정 / 수정 기능
- [x] 일정 취합 (평가위원선정일로부터 2주 이내 평일, 10·14·15·16시 중 선택)
- [x] 평가위원 일정 입력 현황 조회, 일정 확정 (회의장소 포함)
- [x] 일정공지 화면 (안건, 회의개요, 진행순서, 관련자료)
- [x] 사전질의응답 (평가위원·IT관리자 질의 등록/수정 → 추진부서 담당자 답변)
- [x] **협의회 개최** (SCHEDULED → IN_PROGRESS, `PATCH /{asctId}/start`)

**Step 3. 협의회 개최 (평가위원/IT관리자)**
- [x] 평가의견 작성 (타당성 점검 6항목 점수 + 타당성 검토 의견)
- [x] 결과서 작성 (IT기획부 담당자, 1page: 일정공지 내용, 2page: 평균점수/종합의견)
- [x] 결과서 검토 확인 (평가위원 확인 버튼)
- [x] 결과서 확정 (IT관리자, RESULT_WRITING → RESULT_REVIEW)
- [x] 결과보고 결재 (IT기획팀장 → IT기획부장, 포털 내 결재) — FR-14 완료 (2026-05-02)

**백엔드 API**
- [x] 타당성검토표 CRUD API (`/api/council/{asctId}/feasibility`)
- [x] 성과지표 CRUD API
- [x] 평가위원 관리 API (`/api/council/{asctId}/committee`)
- [x] 일정 취합 API (`/api/council/{asctId}/schedule`)
- [x] 사전질의응답 API (`/api/council/{asctId}/qna`) — 등록/수정/답변
- [x] **협의회 개최 API** (`PATCH /api/council/{asctId}/start`)
- [x] 평가의견 API (`/api/council/{asctId}/evaluation`)
- [x] 결과서 API (`/api/council/{asctId}/result`)
- [x] 결재 처리 API (기존 전자결재 모듈 활용)
...
| ID | 요구사항 | 우선순위 | 상태 |
|----|---------|---------|------|
| FR-01 | 접속자 소속부서(BBR_C)와 프로젝트 테이블의 주관부서 일치 사업만 표출 | High | ✅ Done |
| FR-02 | 타당성검토표 임시저장 (언제든지 이어서 작성 가능) | High | ✅ Done |
| FR-03 | 타당성검토표 작성완료 시 상태 전이 및 팀장 결재 요청 | High | ✅ Done |
| FR-04 | 사업계획서 첨부파일 필수 (hwp, hwpx, pdf만 허용) | High | ✅ Done |
| FR-05 | 심의 유형 선택에 따른 당연위원 자동 매핑 (직원정보 테이블 TEM_C, PT_C 기준) | High | ✅ Done |
| FR-06 | 직원검색 팝업으로 소집위원/간사 추가 | Medium | ✅ Done |
| FR-07 | 평가위원 일정 입력 (평가위원선정일+2주 이내 평일, 4개 시간대 중 선택) | High | ✅ Done |
| FR-08 | 평가위원 전원 일정 입력 완료 시 IT관리자에게 일정확정 버튼 활성화 | Medium | ✅ Done |
| FR-09 | 일정공지 화면 (안건/회의개요/진행순서/관련자료) | Medium | ✅ Done |
| FR-10 | 사전질의응답 (참석위원 질의 등록/수정 → 추진부서 답변) | Medium | ✅ Done |
| FR-10a | **협의회 개최** (SCHEDULED → IN_PROGRESS 상태 전이) | High | ✅ Done (신규) |
| FR-11 | 평가의견 작성 (타당성 점검 6항목, 1~2점 시 검토의견 필수) | High | ✅ Done |
| FR-12 | 결과서 2page: 전체 평가위원 점수 평균 자동 계산 | High | ✅ Done |
| FR-13 | 결과서 검토 확인 (평가위원 전원) + 결과서 확정 (IT관리자) | Medium | ✅ Done |
| FR-14 | 결과보고 결재 (IT기획팀장 → IT기획부장 순차 결재) + 추진부서 통보 | High | ✅ Done |
| FR-15 | 전 단계 진행상태 사업 목록에서 실시간 표출 | Medium | ✅ Done |
| FR-16 | 성과지표 동적 추가/삭제 (최소 1개 필수) | Medium | ✅ Done |
...
- [x] FR-01~FR-16 모든 기능 요구사항 구현 완료
- [x] 소관부서 담당자 → 팀장 결재 → IT관리자 → 평가위원 → IT관리자 결재까지 E2E 흐름 동작
- [x] 기존 프로젝트 테이블, 사용자 테이블 데이터 정상 연동 확인
- [x] 파일 업로드/다운로드 정상 동작 (hwp, hwpx, pdf)
- [ ] 단위 테스트 작성 (핵심 서비스 로직) — 별도 이슈 예정

### 4.2 Quality Criteria

- [x] 빌드 성공 (`npm run build`, `./gradlew build`)
- [x] 기존 테스트 전부 통과
- [x] TypeScript 오류 없음 (council 관련)
- [x] 한글 주석 필수 (CLAUDE.md 4.1 준수)
...
→ RESULT_REVIEW(결과서검토) → FINAL_APPROVAL(결과결재)
→ RESULT_APPROVAL_PENDING(결과보고 결재 중) → COMPLETED(완료)
```
...
1. [x] Design 문서 작성 (`/pdca design council`)
2. [x] DB 테이블 DDL 작성 및 생성
3. [x] 백엔드 Entity/Repository/Service/Controller 구현
4. [x] 프론트엔드 페이지/컴포넌트 구현
5. [ ] E2E 테스트 (소관부서→결재→관리자→평가위원→완료 흐름) — 별도 이슈 예정
6. [x] FR-14 결과보고 결재 요청 버튼 UI (result/[id].vue IT관리자 화면) — 완료 (2026-05-02)
7. [ ] 단위 테스트 작성 (핵심 서비스 로직)
...
| 1.0 | 2026-04-26 | FR-01~FR-16 구현 완료 반영. 협의회 개최(FR-10a) 신규 추가. 완료 체크 업데이트 | ssuno |
| 1.1 | 2026-05-02 | FR-14 완료 반영. RESULT_APPROVAL_PENDING 상태 추가. 추진부서 통보 기능. 탭 제목 개선. | ssuno |
